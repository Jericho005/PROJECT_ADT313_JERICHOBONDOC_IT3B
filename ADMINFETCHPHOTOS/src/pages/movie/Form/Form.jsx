...
const Form = () => {
  ...
  const handleSelectMovie = (movie) => {
    ...
    setFormData({
      ...
      videos: [], // Initialize videos as an empty array (to be updated after fetch)
      cast: [], // Initialize cast as an empty array (to be updated after fetch)
    });
    setError("");
...
.then(response => {
  ...
  setFormData(prevData => ({
    ...prevData,
    videos: videoList, // Save videos to formData
  }));
})
.catch(() => {
  setError("Unable to load videos. Please try again later.");
    ...
    .then(response => {
      ...
      setFormData(prevData => ({
        ...prevData,
        cast: castList, // Save cast to formData
      }));
    })
    .catch(() => {
  ...
  const handleSave = async () => {
    ...
    const data = {
      ...
      videos: formData.videos.map(video => {
        return {
          // Store video IDs or direct URLs to videos, avoid YouTube analytics URL
          videoId: video.id,
          videoUrl: video.url,
        };
      }), // Correctly map video data
    ...
    try {
      await axios({
        ...
        url: movieId ? `/movies/${movieId}` : "/movies", // Your backend endpoint for movie save/update
        data: data,
        headers: {
